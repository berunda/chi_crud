...
type WorkoutEntry struct {
	ID              int      `json:"id"`
	ExerciseName    string   `json:"ecercise_name"`
	Sets            int      `json:"sets"`
	Reps            *int     `json:"reps"`
...
func (pg *PostgresWorkoutStore) CreateWorkout(workout *Workout) (*Workout, error) {
	...
	RETURNING id
	`
	err = tx.QueryRow(query, workout.Description, workout.DurationMinutes, workout.CaloriesBurned).Scan(&workout.ID)
	if err != nil {
		return nil, err
	}
	...
	for _, entry := range workout.Entries {
		query := `
		INSERT INTO workout_entries (workout_id, exercise_name, sets, repos, duration_seconds, weight, notes, order_index)
